...
function showDateTimePicker(availableSlots) {
    // ซ่อนส่วนเลือกคอร์ส
    $('#courseSection').hide();

    // แสดงส่วนเลือกวันและเวลา
    $('#dateTimeSection').show();

    // กำหนดค่าให้กับ flatpickr สำหรับเลือกวันที่
    flatpickr("#booking_date", {
        enable: availableSlots.map(slot => slot.date),
        dateFormat: "Y-m-d",
...
function updateTimeSlots(selectedDate, availableSlots) {
    var timeSlots = availableSlots.find(slot => slot.date === selectedDate).times;
    var timeSlotsHtml = '';

    timeSlots.forEach(function(slot) {
        timeSlotsHtml += `
            <div class="col-md-3 mb-2">
                <button type="button" class="btn btn-outline-primary time-slot w-100" data-time="${slot.time}" data-room="${slot.room_id}">
                    ${slot.time}
                </button>
            </div>
        `;
    ...
    // เพิ่ม event listener สำหรับปุ่มเลือกเวลา
    $('.time-slot').on('click', function() {
        $('.time-slot').removeClass('selected');
        $(this).addClass('selected');
        $('#booking_time').val($(this).data('time'));
        $('#selected_room_id').val($(this).data('room'));
    });
}
